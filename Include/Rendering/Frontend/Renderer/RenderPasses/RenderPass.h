...
public:
  ...
  virtual void Init(const Platform::AppInfo& appInfo, ResourceManager* resourceManager,
    const ConfigDataInterface& dependencies = ConfigDataInterface()) = 0;
  virtual void Render(const SceneRenderInfo& sceneRenderInfo, ResourceManager* resourceManager,
...
public:
  ...
  {
    const typename T::ConfigData& data = static_cast<const typename T::ConfigData&>(dependencies);
