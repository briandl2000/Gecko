...
namespace gecko {
  ...
  struct JobHandle
  {
    ...
    bool operator==(const JobHandle& other) const noexcept { return Id == other.Id; }
    bool operator!=(const JobHandle& other) const noexcept { return Id != other.Id; }
  };
  ...
  struct IJobSystem
  {
    ...
    GECKO_API virtual JobHandle Submit(JobFunction job, JobPriority priority = JobPriority::Normal, Category category = Category{0}) noexcept = 0;
    
    GECKO_API virtual JobHandle Submit(JobFunction job, const JobHandle* dependencies, u32 dependencyCount, JobPriority priority = JobPriority::Normal, Category category = Category{0}) noexcept = 0;
  ...
  GECKO_API inline JobHandle SubmitJob(JobFunction job, JobPriority priority = JobPriority::Normal, Category category = Category{0}) noexcept
  {
  ...
  GECKO_API inline JobHandle SubmitJob(JobFunction job, const JobHandle* dependencies, u32 dependencyCount, JobPriority priority = JobPriority::Normal, Category category = Category{0}) noexcept
  {
    if (auto* jobSystem = GetJobSystem())
      return jobSystem->Submit(std::move(job), dependencies, dependencyCount, priority, category);
    return JobHandle{};
  }
