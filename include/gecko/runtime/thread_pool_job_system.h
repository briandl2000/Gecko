...
namespace gecko::runtime {
  ...
  public:
    ...
    virtual JobHandle Submit(JobFunction job, JobPriority priority = JobPriority::Normal, Category category = Category{0}) noexcept override;
    virtual JobHandle Submit(JobFunction job, const JobHandle* dependencies, u32 dependencyCount, JobPriority priority = JobPriority::Normal, Category category = Category{0}) noexcept override;
    ...
    void SetWorkerThreadCount(u32 count) noexcept { m_RequestedWorkerCount = count; }
  ...
  private:
    struct JobCompare
    {
      ...
      {
        // Higher priority jobs should come first (reverse order for priority_queue)
