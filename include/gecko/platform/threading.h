...
// thread name, sleep, yield. Stateless namespace functions; backed by
// pthread on Linux and Win32 on Windows. Selected at compile time.
//
// For pure-std generic helpers (std::thread::hardware_concurrency wraps,
// hashed std::thread::id, etc.) see gecko/core/utility/thread.h.

namespace gecko::platform {

// Stable, opaque identifier for the calling OS thread. The underlying
// value is the native OS thread id (gettid() on Linux, GetCurrentThreadId
// on Windows) widened to 64 bits.
...
// Set a debugger-visible name on the calling thread. Names longer than
// the platform limit (15 chars on Linux pthread) are truncated. nullptr
// is a no-op.
