...
App::App() noexcept
{
  ...
  config.AppName = "Gecko Platform Example";
...
App::~App() noexcept
{
  ...
  m_MovedSub.Reset();
  m_ResizedSub.Reset();
  m_StateChangedSub.Reset();
  for (gecko::usize index = 0; index < m_SpawnedWindowCount; ++index)
  {
    const auto h = m_SpawnedWindows[index];
    if (Windows().IsWindowAlive(h))
      Windows().DestroyWindow(h);
  }
  m_SpawnedWindowCount = 0;
...
void App::CreateMainWindow() noexcept
{
...
void App::SubscribeWindowEvents() noexcept
{
  ...
  m_CloseSub = gecko::SubscribeEvent(
      ...
      [](void* user, const gecko::EventMeta&, gecko::EventView view) {
        ...
        for (gecko::usize index = 0; index < self->m_SpawnedWindowCount; ++index)
        {
          if (self->m_SpawnedWindows[index] == p->Window)
          {
            GECKO_INFO(Main_Label, "Child window {} closed", static_cast<unsigned long long>(p->Window.Id));
            Windows().DestroyWindow(p->Window);
            for (gecko::usize move = index + 1; move < self->m_SpawnedWindowCount; ++move)
              self->m_SpawnedWindows[move - 1] = self->m_SpawnedWindows[move];
            --self->m_SpawnedWindowCount;
            break;
          }
        }
      },
  ...
  m_FocusSub = gecko::SubscribeEvent(
      ...
      [](void*, const gecko::EventMeta&, gecko::EventView view) {
        ...
        GECKO_INFO(Main_Label, "WindowFocus: id={} focused={}", static_cast<unsigned long long>(p->Window.Id),
  ...
  m_MovedSub = gecko::SubscribeEvent(
      ...
      [](void*, const gecko::EventMeta&, gecko::EventView view) {
        ...
        GECKO_INFO(Main_Label, "WindowMoved: id={} pos=({}, {})", static_cast<unsigned long long>(p->Window.Id), p->X,
                   p->Y);
      },
      nullptr);
  ...
  m_ResizedSub = gecko::SubscribeEvent(
      ...
      [](void*, const gecko::EventMeta&, gecko::EventView view) {
        ...
        GECKO_INFO(Main_Label, "WindowResized: id={} size={}x{}", static_cast<unsigned long long>(p->Window.Id),
                   p->Width, p->Height);
      },
      nullptr);
  ...
  m_StateChangedSub = gecko::SubscribeEvent(
      ...
      [](void*, const gecko::EventMeta&, gecko::EventView view) {
        ...
        GECKO_INFO(Main_Label, "WindowStateChanged: id={} {} -> {}", static_cast<unsigned long long>(p->Window.Id),
...
void App::PollInput() noexcept
{
  ...
  for (const auto& b : Buttons)
  {
    if (input->WasMouseButtonPressed(b.Btn))
    {
      ...
      GECKO_INFO(Main_Label, "Input: {} mouse pressed at ({}, {})", b.Name, p.X, p.Y);
    }
    if (input->WasMouseButtonReleased(b.Btn))
    {
      auto p = input->GetMousePosition();
      GECKO_INFO(Main_Label, "Input: {} mouse released at ({}, {})", b.Name, p.X, p.Y);
    }
  }

  if (const float scrollY = input->GetMouseScrollY(); scrollY != 0.0f)
    GECKO_INFO(Main_Label, "Input: scrollY={:.2}", static_cast<double>(scrollY));
  ...
  {
    GECKO_INFO(Main_Label, "Input: focused window {} -> {}",
  ...
  {
    GECKO_INFO(Main_Label, "Input: hovered window {} -> {}",
...
void App::HandleSpawnKey(gecko::platform::KeyCode key) noexcept
{
  using namespace gecko::platform;
  WindowDesc desc;
  desc.Mode = WindowMode::Windowed;
  ...
  case KeyCode::D4:
    ...
    break;
  case KeyCode::W:
    if (m_SpawnedWindowCount == 0)
    {
      GECKO_INFO(Main_Label, "No spawned windows to close");
      return;
    }
    {
      WindowHandle last = m_SpawnedWindows[m_SpawnedWindowCount - 1];
      GECKO_INFO(Main_Label, "Closing last spawned window (id={})", static_cast<unsigned long long>(last.Id));
      Windows().DestroyWindow(last);
      --m_SpawnedWindowCount;
    }
    return;
  default:
    return;
  ...
  if (h.IsValid())
  {
    if (m_SpawnedWindowCount == MaxSpawnedWindows)
    {
      Windows().DestroyWindow(h);
      GECKO_WARN(Main_Label, "Spawn limit reached");
      return;
    }
    m_SpawnedWindows[m_SpawnedWindowCount++] = h;
    GECKO_INFO(Main_Label, "Spawned '{}' (id={})", desc.Title, static_cast<unsigned long long>(h.Id));
  }
}

void App::HandleMainWindowKey(gecko::platform::KeyCode key) noexcept
{
  using namespace gecko::platform;
  ...
  case KeyCode::D: {
    ...
    GECKO_INFO(Main_Label, "Decorations: {} -> {}", BoolStr(decorated), BoolStr(!decorated));
    return;
  }
  case KeyCode::R: {
    const bool resizable = windows.IsResizable(main);
    windows.SetResizable(main, !resizable);
    GECKO_INFO(Main_Label, "Resizable: {} -> {}", BoolStr(resizable), BoolStr(!resizable));
    return;
  }
  case KeyCode::F: {
    ...
    GECKO_INFO(Main_Label, "Window mode: {} -> {}", WindowModeToString(mode), WindowModeToString(next));
    return;
  }
  case KeyCode::T: {
    const bool topmost = windows.IsAlwaysOnTop(main);
    windows.SetAlwaysOnTop(main, !topmost);
    GECKO_INFO(Main_Label, "Always on top: {} -> {}", BoolStr(topmost), BoolStr(!topmost));
    return;
  }
  case KeyCode::M: {
    ...
    GECKO_INFO(Main_Label, "Window state: {} -> {}", WindowStateToString(st), WindowStateToString(next));
    return;
  }
  default:
    return;
...
void App::HandleTitleBarButtonsKey(gecko::platform::KeyCode key) noexcept
{
  using namespace gecko::platform;
  auto& windows = Windows();
  const WindowHandle main = m_MainWindow;
  ...
  case KeyCode::D5: {
    ...
    GECKO_INFO(Main_Label, "Close button: {}", BoolStr(Any(next & WindowButtons::Close)));
    return;
  }
  case KeyCode::D6: {
    const WindowButtons next =
    ...
    windows.SetWindowButtons(main, next);
    GECKO_INFO(Main_Label, "Minimize button: {}", BoolStr(Any(next & WindowButtons::Minimize)));
    return;
  }
  case KeyCode::D7: {
    const WindowButtons next =
    ...
    windows.SetWindowButtons(main, next);
    GECKO_INFO(Main_Label, "Maximize button: {}", BoolStr(Any(next & WindowButtons::Maximize)));
    return;
  }
  case KeyCode::D8:
...
void App::HandleSizeConstraintsKey(gecko::platform::KeyCode key) noexcept
{
  using namespace gecko::platform;
  auto& windows = Windows();
  if (key == KeyCode::D9)
...
void App::PrintWindowInfo() noexcept
{
  using namespace gecko::platform;
  auto& windows = Windows();
  const WindowHandle main = m_MainWindow;
  ...
  GECKO_INFO(Main_Label, "  Size:          {}x{}", size.Width, size.Height);
  GECKO_INFO(Main_Label, "  Position:      ({}, {})", pos.X, pos.Y);
  GECKO_INFO(Main_Label, "  DPI:           {} (scale {:.2})", dpi.Dpi, static_cast<double>(dpi.Scale));
  GECKO_INFO(Main_Label, "  Native:        backend={} handle={}", static_cast<unsigned>(native.Backend), native.Handle);
  GECKO_INFO(Main_Label, "  Mode:          {}", WindowModeToString(windows.GetWindowMode(main)));
  GECKO_INFO(Main_Label, "  State:         {}", WindowStateToString(windows.GetWindowState(main)));
  GECKO_INFO(Main_Label, "  Decorated:     {}", BoolStr(windows.IsDecorated(main)));
  GECKO_INFO(Main_Label, "  Resizable:     {}", BoolStr(windows.IsResizable(main)));
  GECKO_INFO(Main_Label, "  Always on top: {}", BoolStr(windows.IsAlwaysOnTop(main)));
  GECKO_INFO(Main_Label, "  Buttons:       close={} min={} max={}", BoolStr(Any(btns & WindowButtons::Close)),
             BoolStr(Any(btns & WindowButtons::Minimize)), BoolStr(Any(btns & WindowButtons::Maximize)));
}

void App::PrintInputSnapshot() noexcept
{
  using gecko::platform::KeyCode;
  using gecko::platform::MouseButton;
  auto* input = gecko::platform::GetInput();
  ...
  GECKO_INFO(Main_Label, "  Mouse:    pos=({}, {}) delta=({}, {}) scrollY={:.2}", pos.X, pos.Y, delta.X, delta.Y,
             static_cast<double>(input->GetMouseScrollY()));
  GECKO_INFO(Main_Label, "  Buttons:  L={} R={} M={}", input->IsMouseButtonDown(MouseButton::Left) ? "down" : "up",
             input->IsMouseButtonDown(MouseButton::Right) ? "down" : "up",
             input->IsMouseButtonDown(MouseButton::Middle) ? "down" : "up");
  GECKO_INFO(Main_Label, "  Modifiers: shift={} ctrl={}", BoolStr(shift), BoolStr(ctrl));
  GECKO_INFO(Main_Label, "  Focused={} Hovered={}", static_cast<unsigned long long>(focused.Id),
             static_cast<unsigned long long>(hovered.Id));
}
