...
- Use Gecko's `AtomicU32`, `SpinMutex`, `Mutex`, `ConditionVariable`, `Semaphore`, `Thread`, `LockGuard`, and `Defer` primitives instead of compiler or OS synchronization directly. Platform selection belongs in the platform implementation file, not at each call site.
