...
[[nodiscard]] MeshHandle CreateMesh(ResourceManager* resourceManager, const tinygltf::Model& gltfModel,
...
SceneHandle GLTFSceneLoader::LoadNodeBasedScene(const std::string& pathString, ApplicationContext& ctx)
{
	...
	u32 sceneIdx = ctx.GetSceneManager()->CreateScene<NodeBasedScene>(path.filename().string());
