...
namespace gecko::runtime {
  ...
  bool ThreadPoolJobSystem::Init() noexcept
  {
    ...
    // Note: Logger may not be available yet during JobSystem initialization (JobSystem comes before Logger)
    // Use direct fprintf for initialization messages to avoid dependency issues
    std::fprintf(stderr, "[Gecko] Initializing ThreadPoolJobSystem with %u worker threads\n", workerCount);
    ...
    try
    {
      ...
      {
        m_WorkerThreads.emplace_back(&ThreadPoolJobSystem::WorkerThreadFunction, this);
      }
  ...
  void ThreadPoolJobSystem::Shutdown() noexcept
  {
    ...
    // Use direct fprintf during shutdown since logger will be shut down after job system
  ...
  JobHandle ThreadPoolJobSystem::Submit(JobFunction job, JobPriority priority, Category category) noexcept
  {
    if (!m_Initialized || !job) return JobHandle{};
    
    JobHandle handle = GenerateJobHandle();
    auto jobPtr = std::make_shared<Job>(std::move(job), priority, category, handle);
    ...
    {
      GECKO_TRACE(categories::Runtime, "Submitted job %llu with priority %d", handle.Id, static_cast<int>(priority));
    }
    return handle;
  }

  JobHandle ThreadPoolJobSystem::Submit(JobFunction job, const JobHandle* dependencies, u32 dependencyCount, JobPriority priority, Category category) noexcept
  {
    if (!m_Initialized || !job) return JobHandle{};
    ...
    if (category != categories::Logger)
    {
      GECKO_TRACE(categories::Runtime, "Submitted job %llu with %u dependencies", handle.Id, dependencyCount);  
    }
    return handle;
  }
  ...
  void ThreadPoolJobSystem::ProcessJobs(u32 maxJobs) noexcept
  {
    ...
    for (u32 processed = 0; processed < maxJobs; ++processed)
    {
      ...
      try
      {
        ...
        GECKO_TRACE(categories::Runtime, "Completed job %llu on main thread", job->Handle.Id);
      }
      catch (...)
      {
        GECKO_ERROR(categories::Runtime, "Job %llu threw an exception", job->Handle.Id);
        job->Completed.store(true, std::memory_order_release);
      }
  ...
  void ThreadPoolJobSystem::WorkerThreadFunction() noexcept
  {
    ...
    while (!m_Shutdown.load(std::memory_order_acquire))
    {
      ...
      if (!job)
      {
        ...
        {
          return m_Shutdown.load(std::memory_order_acquire) || !m_JobQueue.empty();
        });
        continue;
      }
      ...
      try
      {
        ...
        GECKO_TRACE(categories::Runtime, "Completed job %llu on worker thread %u", job->Handle.Id, threadId);
      }
      catch (...)
      {
        GECKO_ERROR(categories::Runtime, "Job %llu threw an exception on worker thread %u", job->Handle.Id, threadId);
        job->Completed.store(true, std::memory_order_release);
      }
  ...
  std::shared_ptr<Job> ThreadPoolJobSystem::GetNextReadyJob() noexcept
  {
    ...
    if (m_JobQueue.empty()) return nullptr;
