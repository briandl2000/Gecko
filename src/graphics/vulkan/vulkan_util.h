...
[[nodiscard]] inline VkColorComponentFlags ToVkColorWriteMask(u8 mask) noexcept
{
  ...
  if (mask & static_cast<u8>(ColorWriteMask::Red))   out |= VK_COLOR_COMPONENT_R_BIT;
  if (mask & static_cast<u8>(ColorWriteMask::Green)) out |= VK_COLOR_COMPONENT_G_BIT;
  if (mask & static_cast<u8>(ColorWriteMask::Blue))  out |= VK_COLOR_COMPONENT_B_BIT;
  if (mask & static_cast<u8>(ColorWriteMask::Alpha)) out |= VK_COLOR_COMPONENT_A_BIT;
  return out;
}
...
[[nodiscard]] inline VkShaderStageFlags ToVkShaderStage(ShaderType s) noexcept
{
  switch (s)
  {
    ...
    case ShaderType::All:     return VK_SHADER_STAGE_ALL;
  }
    ...
    ResourceType t, bool isCompute) noexcept
{
  ...
  switch (t)
  {
    ...
    case ResourceType::RWTexture:          return VK_DESCRIPTOR_TYPE_STORAGE_IMAGE;
    case ResourceType::ConstantBuffer:     return VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    case ResourceType::StructuredBuffer:   return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    case ResourceType::RWStructuredBuffer: return VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
