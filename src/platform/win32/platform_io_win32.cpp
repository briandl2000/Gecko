...
// Convert a forward-slash UTF-8 PathView to a null-terminated wide
// string with backslashes (the canonical Win32 form). Empty input
...
::std::wstring ToWide(PathView path) noexcept
{
  if (path.Empty())
    return {};
  auto sv = path.View();
  // First pass: required wide length (with backslash translation).
...
// Convert a wide path back to UTF-8 with forward slashes.
...
class Win32PlatformIO final : public IPlatformIO
{
public:
  ...
  ::std::optional<FileStat> Stat(PathView path) noexcept override
  {
    ...
    // Convert FILETIME (100ns since 1601) -> seconds since unix epoch.
    ULARGE_INTEGER ft {};
  ...
  MappedFile Map(PathView path) noexcept override
  {
    ...
    if (w.empty())
      return {};
    HANDLE file =
        ::CreateFileW(w.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr,
                      OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
  ...
  bool CreateDir(PathView path, bool recursive) noexcept override
  {
    ...
    for (::std::size_t i = 1; i <= w.size(); ++i)
    {
      if (i == w.size() || w[i] == L'\\')
      {
        ...
        w[i] = L'\0';
        // Skip "C:" with no trailing component.
...
{
  return ::gecko::CreateUnique<Win32PlatformIO>();
}
